...
@else
    <div class="{{ data_get($theme, 'baseClass') }}" style="{{ data_get($theme, 'baseStyle') }}">
        <div>
            <input {{ $defaultAttributes['inputStartAttributes'] }}
                @if ($inline) style="{{ data_get($theme, 'inputStyle') }} {{ data_get($column, 'headerStyle') }}" @endif
                type="text" class="{{ $filterClasses }} tb-rounded"
                placeholder="{{ $placeholder['min'] ?? __('Min') }}">
        </div>
        <div class="me-1">
            <input {{ $defaultAttributes['inputEndAttributes'] }}
                @if ($inline) style="{{ data_get($theme, 'inputStyle') }} {{ data_get($column, 'headerStyle') }}" @endif
                type="text" class="form-control form-control-sm tb-rounded"

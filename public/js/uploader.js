...
$(document).on("click", ".remove-btn", function (event) {
    event.preventDefault();
    const previewContainer = $(this).closest("[data-preview]");
    if (previewContainer.length) {
        previewContainer.empty().removeClass("file-item border border-1");
    }
    const fileInput = previewContainer
        .closest(".form-group")
        .find("[data-file-input]");
    if (fileInput.length) {
        fileInput.val("");
    }
    const formGroup = previewContainer.closest(".form-group");
    const fileCountDisplay = formGroup.find("[data-file-count]");
    if (fileCountDisplay.length) {
        fileCountDisplay.text("Choose File");
    }
});
...
function fetchFiles(type = "", query = "", shareids = "", checkTypes = "") {
    ...
    $.post({
        ...
        success: function (response) {
            if (response.files != "") {
                displayFiles(response.files, checkType, shareid.val(), shareid);
                selectFileCount();
            }
            currentPage = 1;
            setupPagination();
...
function displayFiles(fileData, types = "", selectedIds = "", selectInput) {
    files = fileData;
    type = types;
    shareid = selectInput;
    let inputSelect = selectInput;
    ...
    files.sort((a, b) => {
        const aSelected = selectedIdArray.includes(a.id.toString());
        const bSelected = selectedIdArray.includes(b.id.toString());
        return aSelected === bSelected ? 0 : aSelected ? -1 : 1; // aSelected first, then bSelected
    });
            ...
            `<div class="col-lg-2 col-md-3 col-4 px-1">
              <label class="w-100 pointer" for="radio-${file.id}" >
                <input type="${type != "" ? type : "radio"}" class="d-none" 
                  data-value="'${file.external_link}','${fileType}','${
                file.id
            }',${shareid},${file.original_name},${file.file_size}" 
                  name="file-selection" value="${
                      file.id
                  }" ${isChecked} id="radio-${file.id}">
                <div class="file-item ${fileActive}" data-file-name="${
...
function handleSelectFile() {
    ...
    $('input[name="file-selection"]:checked').each(function () {
        ...
        if (fileData.length >= 3) {
            ...
            const fileSize = fileData[5];
            selectedIds.push(fileId);
            let chooseContainer = fileCount;
            let choosedFile = $(chooseContainer);
            //choosedFile.text(fileName);
            fileNameList.push(fileName);
            displayFileInPreview(
                uploadID,
                fileUrl,
                fileType,
                fileName,
...
function displayFileInPreview(id, link, type, name, size) {
    const fileUrl = link;
    const fileType = type;
    let innerContent = ""; // Content inside the wrapper
    const previewCon = preview; // Assuming 'preview' is a predefined container reference

    // Determine inner content based on file type
    if (fileType === "image") {
        innerContent = `
            <img src="${fileUrl}" alt="File preview" class="fit-image border-bottom border-light" 
                style="width: 150px; height: 130px; border-radius: 5px;" />
        `;
    } else if (fileType === "video") {
        innerContent = `
            <video autoplay muted loop class="shadow" 
                style="width: 150px; height: 130px; border-radius: 5px;">
        ...
        innerContent = `
            <iframe src="${fileUrl}" class="shadow" 
                style="width: 150px; height: 130px; border-radius: 5px;"></iframe>
        `;
        ...
        innerContent = "<p>No preview available</p>";
    }

    // Common wrapper
    const content = `
        <div class="file-item bg-white h-auto overflow-hidden border-light border-1" 
            style="max-height:150pxpx; width:150px;">
            ${innerContent}
            <button type="button" class="remove-btn">×</button>
            <div class="p-1 text-start d-none">
                <span class="text-muted fss-12 text-overflow-ellipsis">${name}</span>
            </div>
        </div>
    `;
    ...
    if (previewCon.length) {
        previewCon.empty(); // Clear existing content
        previewCon.removeClass("d-none").append(content); // Add new content
    } else {
...
$(document).ready(function () {
    $(document).on("click", "[data-upload]", function () {
        var $this = $(this);
        var fileInput = $this.find("[data-file-input]");
        var type = $this.data("type");
        var previewContainer = $($this.data("preview"));
        var multiple = $this.data("multiple");
        fetchFiles(type, "", fileInput, multiple);
        fileCount = $this.find("[data-file-count]");
        preview = $this.next("[data-preview]");
    });
});
